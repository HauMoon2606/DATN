...
class GulandSpiderSpider(scrapy.Spider):
    ...
    async def parse_real_estate_page(self, response):
        ...
        bds_item['address'] = {
            'full_address': response.css('div.dtl-stl__row')[0].css('span::text').get(),
            'province': province,
            'district': district,
            'ward': None
        }
        ...
        contact_info = {
            'name': response.css('div.dtl-aut__cxt h5.dtl-aut__tle::text').get(),
            'phone': [phone.strip() for phone in response.css('#modal-contact-content > div > div > div > div.zl-wrp__cnt > div.zl-wrp__num > a::text').getall()]
        }
    ...
    def _errback(self, failure):
        ...
        if page:
            try:
                self.logger.warning("Closing playwright page due to error...")
                self.crawler.engine.close_spider(self, "playwright_page_error")
            except Exception as e:
                self.logger.error(f"Failed to close playwright page: {e}")
